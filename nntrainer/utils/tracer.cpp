...
/**
 * Copyright (C) 2022 Jiho Chu <jhoon.it.lee@samsung.com>
 *
 * @file   trace.cpp
...
MemoryTracer::MemoryTracer(const std::string &name, bool flush) :
  ...
  trace_info.emplace_back(getMemoryUsage(), "start");
  if (flush)
    writeToFile(outputFileName(name), trace_info);
}

MemoryTracer::~MemoryTracer() { writeToFile(outputFileName(name), trace_info); }

Tracer &MemoryTracer::tracePoint(const std::string &msg) {
  trace_info.emplace_back(getMemoryUsage(), msg);
  if (flush)
    writeToFile(outputFileName(name), trace_info);
...
TimeTracer::TimeTracer(const std::string &name, bool flush) :
  Tracer(name),
  flush(flush) {
  std::ofstream ifs(outputFileName(name), std::fstream::trunc);
  trace_info.emplace_back(getTimeStamp(), "start");
  if (flush)
    writeToFile(outputFileName(name), trace_info);
}

TimeTracer::~TimeTracer() { writeToFile(outputFileName(name), trace_info); }

Tracer &TimeTracer::tracePoint(const std::string &msg) {
  trace_info.emplace_back(getTimeStamp(), msg);
  if (flush)
    writeToFile(outputFileName(name), trace_info);
